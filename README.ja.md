...
CPU/GPU/メモリ監視、メニューバーインジケーター（CPU/GPU はスピナー、メモリは充填ゲージ）、
クリックで開くパネル（ライブ値ゲージ＋メモリドーナツ＋履歴グラフのステータス面が、めくると
設定面へ反転）まで実装・署名・notarize 済み。
...
- メモリ: メニューバーへの充填ゲージ表示、枠（丸/四角）、色（単色 ／ 使用率連動グラデ）。
  グラデはメモリ専用の4色構成（青→緑→オレンジ→赤）: ほぼ未使用は「冷えた」青、
  健康的な中域のスイートスポットは緑、高域だけが暖色に向かいます
- ログイン時に起動

パネル下部には macOS の**アクティビティモニタ**を開くボタンがあります。
